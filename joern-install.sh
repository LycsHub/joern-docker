...
if [ $NO_DOWNLOAD = true ]; then
  ...
  if [ "$JOERN_VERSION" = "" ]; then
    curl -L "https://ghproxy.com/https://github.com/ShiftLeftSecurity/joern/releases/latest/download/joern-cli.zip" -o "$SCRIPT_ABS_DIR/joern-cli.zip"
  else
    curl -L "https://ghproxy.com/https://github.com/ShiftLeftSecurity/joern/releases/download/$JOERN_VERSION/joern-cli.zip" -o "$SCRIPT_ABS_DIR/joern-cli.zip"
  fi
fi
...
if [ $INTERACTIVE = false ] && [ "$(whoami)" != "root" ]; then
  ...
  echo "==============================================================="
    ...
    if [ -n "${JOERN_LINK_DIR+dummy}" ]; then
	    ...
	    ln -sf "$JOERN_INSTALL_DIR"/joern-cli/joern "$JOERN_LINK_DIR" || true # 非docker环境 正常执行需要root权限 加sudo
	    ln -sf "$JOERN_INSTALL_DIR"/joern-cli/joern-parse "$JOERN_LINK_DIR" || true
	    ln -sf "$JOERN_INSTALL_DIR"/joern-cli/c2cpg.sh "$JOERN_LINK_DIR" || true
	    ln -sf "$JOERN_INSTALL_DIR"/joern-cli/ghidra2cpg "$JOERN_LINK_DIR" || true
	    ln -sf "$JOERN_INSTALL_DIR"/joern-cli/jssrc2cpg.sh "$JOERN_LINK_DIR" || true
	    ln -sf "$JOERN_INSTALL_DIR"/joern-cli/javasrc2cpg "$JOERN_LINK_DIR" || true
	    ln -sf "$JOERN_INSTALL_DIR"/joern-cli/jimple2cpg "$JOERN_LINK_DIR" || true
	    ln -sf "$JOERN_INSTALL_DIR"/joern-cli/kotlin2cpg "$JOERN_LINK_DIR" || true
	    ln -sf "$JOERN_INSTALL_DIR"/joern-cli/php2cpg "$JOERN_LINK_DIR" || true
	    ln -sf "$JOERN_INSTALL_DIR"/joern-cli/pysrc2cpg "$JOERN_LINK_DIR" || true
...
if [ $INSTALL_DEFAULT_PLUGINS = true ]; then
  ...
  ./joern-scan --updatedb # 这一步可以不执行，如果没有scan需求，scan功能是通用的sast扫描，会从github拉取对应的规则，速度较慢（取消更新可以在运行此shell脚本时加上--without-plugins）
  cd "$CURDIR"
fi
